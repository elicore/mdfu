...
Type to narrow — bare words fuzzy, `key:value` hard-filters via the live `FilterFunc` (`scan → parse → query → rank` on every keystroke). `Enter` prints the selection (pipeable to editors/fzf flows).
...
| `tab` | Toggle multi-select on cursor item. |
| `ctrl+a` | Toggle archived visibility (default hidden). |
| `ctrl+p` | Toggle preview pane (first 30 body lines + Title/Path/Type/Tags/Status; side-by-side when ≥100 cols). |
